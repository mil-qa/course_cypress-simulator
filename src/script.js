document.addEventListener("DOMContentLoaded", () => {
  ...
  const cypressCommands = {
    ...
    "cy.clock":
      "cy.clock() overrides native global functions related to time allowing them to be controlled synchronously via cy.tick() or the yielded clock object",
    ...
    "cy.on":
      "Liston to a single browser event, which is unbound when the test ends",
    ".and": "An alias of .should",
    ".as": "Assign an alias for later use",
    ".blur": "Blur a focused element",
    ".check": "Check checkbox(es) or radio(s)",
    ".children":
      "Get the children of each element within a set of DOM elements",
    ...
    "Cypress.ensure":
      "Cypress.ensure is a collection of helper methods for making assertions",
    ...
    "Cypress.SelectorPlayground":
      "The Selector Playground exposes APIs that enable you to: a) Change the default selector strategy; and b) Override the selectors that are returned per element",
    "Cypress.session":
      "Cypress.session is a collection of async session-related helper methods intended to be used alongside the cy.session() command",
    "Cypress.spec":
    ...
    "Cypress.sinon":
      "Cypress automatically includes Sinon.JS and exposes it as Cypress.sinon",
    ...
    beforeEach:
      "Callback function that runs before each test case in a suite or sub-suite",
    after: "Callback function that runs after the test suite",
    afterEach:
      "Callback function that runs after each test case in a suite or sub-suite",
    ".only":
      "A feature that allows you to run only the specified suit, sub-suite, or test case",
    ".skip":
      "A feature that allows you to skip only the specified suite, sub-suite, or test case",
    expect:
  ...
  document.addEventListener("click", (event) => {
    if (
      !sandwichMenu.contains(event.target) &&
  ...
  function runCode() {
    ...
    outputArea.classList.remove("errorOutput");
...
For more commands and details, visit the <a href="https://docs.cypress.io/api/table-of-contents" target="_blank" rel="noopener noreferrer">official Cypress API documentation</a>.`;
        ...
        outputArea.classList.add("errorOutput");
        ...
        if (cypressCommands.hasOwnProperty(command)) {
          switch (command) {
            ...
            case "cy.get":
              if (code.includes("(")) {
                message = `Success:\n\n${code} // Got element by selector ${code
                  .split("(")[1]
                  .replace(")", "")}`;
                outputArea.classList.add("success");
              } else {
            ...
            case "cy.exec":
              if (code.includes("(")) {
                message = `Success:\n\n${code} // Executed system command ${code
                  .split("(")[1]
                  .replace(")", "")}`;
                outputArea.classList.add("success");
              } else {
            ...
            case "cy.fixture":
              if (code.includes("(")) {
                message = `Success:\n\n${code} // Loaded a fixed set of data located in the ${code
                  .split("(")[1]
                  .replace(")", "")} file`;
                outputArea.classList.add("success");
              } else {
            ...
            case "cy.task":
              if (code.includes("(")) {
                message = `Success:\n\n${code} // Executed Node code via the ${code
                  .split("(")[1]
                  .replace(")", "")} task`;
                outputArea.classList.add("success");
              } else {
            ...
            case "cy.readFile":
              if (code.includes("(")) {
                message = `Success:\n\n${code} // Read the ${code
                  .split("(")[1]
                  .replace(")", "")} file and yielded its contents`;
                outputArea.classList.add("success");
              } else {
                message = `Error:\n\nMissing parentheses on \`${code}\` command`;
                outputArea.classList.add("errorOutput");
